...
- missing semantic template sections for forms: 表單用途, 適用場景, 表單結構, 填寫重點, 欄位說明, RAG 查詢摘要
...
{{
  ...
  "triggers": ["3-10 search keywords in Traditional Chinese"],
  ...
  "filling_guide": "Traditional Chinese markdown. For forms use sections: ## 表單用途, ## 適用場景, ## 表單結構, ## 填寫重點, ## 簽核流程, ## 注意事項. For reference_table/diagram/flowchart/org_chart, summarize structure and query dimensions instead of form fields.",
...
class VLMAdapter:
    ...
    async def _enrich(
        ...
        try:
            ...
            format_vars = {"context": context or "None provided"}
            if extra_vars:
    ...
    def _parse_response(self, raw: str, kind: str) -> dict[str, Any]:
        ...
        cleaned_raw = self._repair_json(raw)
