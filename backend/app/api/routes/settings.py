...
async def update_profile_overrides(
    ...
    if update.semantic_output_language is not None:
        if update.semantic_output_language not in [item.value for item in SemanticOutputLanguage]:
            raise HTTPException(400, f"Invalid semantic_output_language: {update.semantic_output_language}")
        current["semantic_output_language"] = update.semantic_output_language
